...
pos <- subset(ccweed, LiBio < 0)
overall <- subset(ccweed, LiBio < 8)
##68 weed density reduced out of 123 = 59% 
##81 weed biomass reduced out of 129 = 63%
##93 out of 124 yield RRs were negative: 75%



##reordering for a scatterplot
ccweed1.o <- ccweed[order(ccweed$DenPer),]
ccweed2.o <- ccweed[order(ccweed$BioPer),]




xyplot(1:nrow(ccweed2.o) ~ BioPer , data = ccweed2.o, 
       cex=1.5, subset = BioPer< 1000,
       groups=cc_termMETH2,
       auto.key=TRUE,
       pch=20,
       xlab= list('Percent Change in Weed Biomass with Cover Crops', cex=1.5),
...
xyplot (LiBio ~ cc_bm_kg.ha, dat=posden, #subset = BioPer < 0,
        type=c("p"), 
...
fit <- lmer(LiDen ~ 1 + (1|Study),
            data=ccweed
            #,weights = Wi add this back in for weights
)
summary(fit)
##need to remove the infinities to get a calc
