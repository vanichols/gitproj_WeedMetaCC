##first attempt at cover crops weed analysis

setwd("C:/Users/abasche2/Dropbox/CC weed meta/analysis")

##for AB home computer
setwd("~/Dropbox/CC weed meta/analysis")

## these are the packages that I tried to use at various points of another analysis, we prob won't need them all
## cancelling out the ones that I don't think that I used
library(lattice)
library(lme4)
library(ggplot2)
#library(lmerTest) not sure why this one isn't working
#library(boot)
#library(devtools)
#install_github("easyGgplot2", "kassambara")
#library(easyGgplot2)
#library(grid)
#library(rmeta) #don't think I used this but maybe useful for MAs
#library (nlme) 
#library (plotrix) 

## follow up from Convo on 3/22
## AB to look more into termination method
## we could try to fit a non linear function to the percent control
## regression coefficients


ccweed <- read.csv("ccweedmeta_v3.csv", na.strings='.', stringsAsFactors=FALSE)
ccweed[ccweed=="NA"]<-"."
##this version was reading the numerics and characters
ccweed$cced_wd.den_num.m2 <- as.numeric(as.character(ccweed$cced_wd.den_num.m2))
ccweed$ctl_wd.den_num.m2 <- as.numeric(as.character(ccweed$ctl_wd.den_num.m2))
ccweed$cced_wd.bm_g.m2 <- as.numeric(as.character(ccweed$cced_wd.bm_g.m2))
ccweed$ctl_wd.bm_g.m2 <- as.numeric(as.character(ccweed$ctl_wd.bm_g.m2))
ccweed$cced_yield_kg.ha <- as.numeric(as.character(ccweed$cced_yield_kg.ha))
ccweed$ctl_yield_kg.ha <- as.numeric(as.character(ccweed$ctl_yield_kg.ha))
ccweed$cc_bm_kg.ha <- as.numeric(as.character(ccweed$cc_bm_kg.ha))
...
##data organizing
##probably want to change the variable names in the excel file or in R
## add study no.
## fix the dates & make sure other variables are similiar (cover crop type, term method)

## change any zeros in the response ratio calc to 0.001
##this approach becomes problematic
## as long as the range is >95% or <95% +/-4 on log scale - then we still included them with the very low number
## beyond +/- 4.6
## values because its not biologically interesting or important
## Another option is to include averages by treatments over years
##Verret et al just got rid of the zero values

ccweed[ccweed==0.0]<-0.001

##count of zeros for wd biomass and wd density
zeros <- subset(ccweed, ctl_wd.bm_g.m2 == 0)
#cced_wd.den_num.m2 ## 6 values
#ctl_wd.den_num.m2 ##34 values
#cced_wd.bm_g.m2 ## 6 values
#ctl_wd.bm_g.m2 ## 34 values
## there are more in the controls than the treatments. which would make us dividing by a small number
## which results in very large RR's
## And ALL from the DeBruin paper
## Gina's advice is to cap the actual response - something that is biologically meaningful
##

##creating a column just to display data from debruin
ccweed$debruin[ccweed$Study ==3] <- "DeBruin"
ccweed$debruin[ccweed$Study < 3] <- "Other"
ccweed$debruin[ccweed$Study > 3] <- "Other"



ccweed$LiDen <- log((ccweed$cced_wd.den_num.m2)/(ccweed$ctl_wd.den_num.m2))
ccweed$DenChange <- (ccweed$cced_wd.den_num.m2)/(ccweed$ctl_wd.den_num.m2)
ccweed$DenPer <- ((exp(ccweed$LiDen))-1)*100

ccweed$LiBio <- log((ccweed$cced_wd.bm_g.m2)/(ccweed$ctl_wd.bm_g.m2))
ccweed$BioChange <- (ccweed$cced_wd.bm_g.m2)/(ccweed$ctl_wd.bm_g.m2)
ccweed$BioPer <- ((exp(ccweed$LiBio))-1)*100

##yield response ratio
ccweed$LiYield <- log((ccweed$cced_yield_kg.ha)/(ccweed$ctl_yield_kg.ha))
ccweed$YieldChange <- (ccweed$cced_yield_kg.ha)/(ccweed$ctl_yield_kg.ha)
ccweed$YieldPer <- ((exp(ccweed$LiYield))-1)*100

##think about weighting
dat1$Wi <- (dat1$Reps * dat1$Reps) / (dat1$Reps + dat1$Reps)
##for counts
length(unique(ccweed$Study))

##counts for overall reduced effect of weed density & biomass
pos <- subset(ccweed, LiDen < 0)
overall <- subset(ccweed, LiDen < 8)
##88 weed density reduced out of 147 = 60% 
##97 weed biomass reduced out of 152 = 64%
##93 out of 124 yield RRs were negative: 75%

##eliminating any values > or < 4 SDs
ccweed3 <- subset(ccweed, LiDen < 5 & LiDen > -5)
ccweed4 <- subset(ccweed, LiBio < 5 & LiBio > -5)

#how many are now reducing weeds
pos <- subset(ccweed3, LiDen < 0) ##88/144 - 61%
pos <- subset(ccweed4, LiBio < 0) ##96/148 - 65%



##reordering for a scatterplot
ccweed1.o <- ccweed[order(ccweed$LiDen),]
ccweed2.o <- ccweed[order(ccweed$LiBio),]

ccweed3.o <- ccweed3[order(ccweed3$LiDen),]
ccweed4.o <- ccweed4[order(ccweed4$LiBio),]



xyplot(1:nrow(ccweed1.o) ~ LiDen , data = ccweed1.o, 
...
xyplot(1:nrow(ccweed2.o) ~ LiBio , data = ccweed2.o, 
       cex=1.5, #subset = BioPer< 1000,
       groups=debruin,
       auto.key=TRUE,
       pch=20,
...
## this seems to nicely show that the winter term is still effective weed control
## clear that mechnical term was not always effective
xyplot(1:nrow(ccweed3.o) ~ LiDen , data = ccweed3.o, 
       cex=1.5, #subset = BioPer< 1000,
       groups=cc_termMETH2,
       auto.key=TRUE,
       pch=20,
...
xyplot(1:nrow(ccweed4.o) ~ LiBio , data = ccweed4.o, 
       cex=1.5, #subset = BioPer< 1000,
       groups=cc_termMETH2,
       auto.key=TRUE,
       pch=20,
...
##count so far for termination methods
#winterkill - 57 - 24%
##herbicide - 77 - 32%
##mechanical - 76 - 32%
##dual - 14 - 6%

termw <- subset(ccweed, cc_termMETH2 == 'W')
termm <- subset(ccweed, cc_termMETH2 == 'M')
termh <- subset(ccweed, cc_termMETH2 == 'H')
termd <- subset(ccweed, cc_termMETH2 == 'D')


## cover crop biomass relationship to percent weed density change

##subset data to not include the 0 cc biomass reporting
posden <- subset(ccweed, cc_bm_kg.ha > 5)
##note that this excludes the Fisk study because those biomass values were so low

xyplot (LiBio ~ cc_bm_kg.ha, dat=ccweed4, #subset = BioPer < 0,
        type=c("p"), 
...
xyplot (LiDen ~ cc_bm_kg.ha, dat=posden, #subset = BioPer < 0,
        type=c("p"), 
        ylab="Log RR Change in Weed Density",
        xlab="Cover Crop Biomass kg/ha",
        groups=cc_termMETH2,
        auto.key=TRUE,
        pch=20,
...
xyplot (LiYield ~ cc_bm_kg.ha|cc_termMETH2, dat=posden, #subset = LiBio < 5, this excludes DeBruin wonky RRs
        type=c("p","r"), 
        ylab="Log RR Change in Weed Biomass",
        xlab="Cover Crop Biomass kg/ha",
        auto.key=TRUE,
...
##what are the high positives?! THESE ARE ALL FROM THE DEBRUIN STUDY!
why <- subset(ccweed, LiBio > 5)


## for stats
##need to add in a study factor
fit <- lmer(LiBio ~ 1 + (1|Study),
            data=ccweed4
            #,weights = Wi add this back in for weights
)
summary(fit)
##looks like a 20% reduction in weed density with a SE of 41%
## and a 55% reduction in weed biomass w/ a SE of 41%


fit <- lmer(LiBio ~ cc_bm_kg.ha + (1|Study),
            data=ccweed4
            #,weights = Wi add this back in for weights
)
summary(fit)
##  HOW DO WE INTERPRET THESE REGRESSION COEFFICIENT VALUES?!

fit <- lmer(LiDen ~ cc_termMETH2 + (1|Study),
            data=ccweed
            #,weights = Wi add this back in for weights
)
summary(fit)

fit <- lmer(LiDen ~ cc_type + (1|Study),
            data=ccweed
            #,weights = Wi add this back in for weights
)
summary(fit)
...
xyplot (cc_bm_kg.ha ~ cc_pDATE_DOY, dat=ccweed,
        groups=cc_type,
        pch=20,
        cex=1.3,
        auto.key=TRUE,
        xlab="Planting DOY",
        ylab="Cover Crop Biomass kg/ha")

xyplot (cc_bm_kg.ha ~ CC_termDATE_DOY, dat=ccweed, 
        groups=cc_type,
        pch=20,
        cex=1.3,
        auto.key=TRUE,
        xlab="Termination DOY",
        ylab="Cover Crop Biomass kg/ha")

##win win graphs
xyplot (LiYield ~ LiDen, dat=ccweed,
        xlab="Log RR Change in Weed Density",
        ylab="Log RR Change in Yield",
        groups=cc_type,
        auto.key=TRUE,
        pch=20,
        cex=1.3,
...
xyplot (LiYield ~ LiBio, dat=ccweed,
        xlab="Log RR Change in Weed Biomass",
        ylab="Log RR Change in Yield",
        groups=cc_type,
        auto.key=TRUE,
        pch=20,
...
pos <- subset(ccweed, LiYield < 0 & LiDen > 0)
overall <- subset(ccweed, LiYield < 8)




##closer look at the yields
ccweed3.o <- ccweed[order(ccweed$LiYield),]

xyplot(1:nrow(ccweed3.o) ~ LiYield , data = ccweed3.o, 
       cex=1.5, 
       groups=crop_follow,
       auto.key=TRUE,
       pch=20,
       xlab= list('Percent Change in Yield with Cover Crops', cex=1.5),
       ylab= list('Observation Rank', cex=1.5),
       scales = list(tck = c(-1,0), cex=1.2),
       panel = function(x,y,...){
         panel.xyplot(x,y, ...)
